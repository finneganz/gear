...
class UserController extends BaseController
{
    ...
    public function showUserPage(string $userName)
    {
        ...
        foreach($userDevices as $genre => $userDevice)
        {
            ...
            $userDevice->maker_name = str_replace('_', ' ', $userDevice->getMaker->maker_name);
        }
    ...
    public function editUser(UserSettingRequest $request)
    {
        ...
        $userDomain = new UserDomain;
        // Config
        $this->userConfig->config = $request->hasFile('config') ? base64_encode(file_get_contents($request->config)) : $this->userConfig->config;
        $this->userConfig->autoexec = $request->hasFile('autoexec') ? base64_encode(file_get_contents($request->autoexec)) : $this->userConfig->autoexec;
